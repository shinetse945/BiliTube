
<p align="center">
<img src="https://raw.githubusercontent.com/shinetse945/BiliTube/main/bilitube/public/icons/logoicon-cropped.png?v=1" width="128">
</p>

<h1 align="center">BiliTube</h1>

<p align="center"><a href="./README.md">中文</a> | English</p>

### This project is a fork built on top of [Danmaku-Kakashi-Ext](https://github.com/Danmaku-Kakashi/Danmaku-Kakashi-Ext)

### Enjoy danmaku (bullet comments) on YouTube™

##### Features

- Search for matching BiliBili videos from within YouTube

- Load BiliBili danmaku onto YouTube videos

##### How to Use

1. Download and extract BiliTube-vX.X.zip.

2. Open chrome://extensions/ and enable "Developer mode".

3. Click "Load unpacked" in the top-left corner.

4. Select the extracted folder.

Do not delete the extracted folder, otherwise the extension will stop working!

##### Technologies Used

- React.js

- Node.js

- Chrome API

- Material UI (MUI)

- [Comment Core Library](https://github.com/jabbany/CommentCoreLibrary) - Licensed under the MIT license

##### Screenshot

<img width="2451" height="1341" alt="image" src="https://github.com/user-attachments/assets/7c039ed4-1fc7-48d8-a61d-801ebecf8107" />

##### Original Project Authors

**Erlan (Leon) Liang**: [GitHub](https://github.com/ErlanLiang) | [LinkedIn](https://www.linkedin.com/in/leon-erlanliang/)

**Jesse Xu**: [GitHub](https://github.com/mgrddsj) | [LinkedIn](https://linkedin.com/in/jessexu)
