...
Danmaku Kakashi 是一个 **Chrome 浏览器扩展（Manifest V3）**，核心功能是在 YouTube 视频上叠加来自 Bilibili 的弹幕。
...
│       └── i18n/                 ← 国际化
│
└── node_modules/                 ← 根目录也有，但主要依赖在 danmaku-kakashi/node_modules/
```
...
| 文件 | 职责 |
|---|---|
| `src/index.js` | **应用入口**。在 YouTube 页面创建 `#danmaku-kakashi-root` 容器，插入到侧边栏 `#secondary-inner`；包裹 ThemeProvider / LanguageProvider / AccessTokenProvider；监听 YouTube SPA 导航事件触发重新渲染；调用 `DanmakuHelper()` 初始化弹幕引擎 |
...
| `DamakuPanel.js` | 弹幕设置面板的**旧版原生 DOM 实现**（纯 JS 操作 innerHTML）。目前未被使用，`App.js` 引用的是 `DanmakuPanel.js` |
...
配置在 `danmaku-kakashi/.env`，通过 `dotenv-webpack` 在编译时注入：
...
### 修改扩展权限或匹配规则
→ 编辑 `danmaku-kakashi/public/manifest.json`，重新构建后自动复制到 `extension/`
...
5. 图片资源通过 webpack 的 `asset/resource` 处理，输出文件名格式为 `[name][hash:8][ext]`
6. 所有跨域请求（Bilibili API、缩略图等）必须通过 background script 代理，content script 受 CORS 限制
